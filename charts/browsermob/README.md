...
```console
$ helm install selenium-browsermob
```

## Introduction

This chart bootstraps a [Selenium](http://www.seleniumhq.org/) deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager. It adds a [Browsermob Proxy](https://bmp.lightbody.net/) for tracking http Requests.
...
```console
$ helm install --name my-release selenium-browsermob
```
...
| `browsermob.image` | The selenium hub image | `qautomatron/docker-browsermob-proxy` |
| `browsermob.tag` | The selenium hub image tag | `2.1.3` |
| `browsermob.pullPolicy` | The pull policy for the hub image | `IfNotPresent` |
| `browsermob.port` | The port the proxy listens on | `9090` |
| `hub.javaOpts` | The java options for the selenium hub JVM, default sets the maximum heap size to 1,000 mb | `-Xmx1000m` |
| `browsermob.resources` | The resources for the hub container, defaults to minimum half a cpu and maximum 500 mb RAM | `{"limits":{"cpu":".5", "memory":"512Mi"}}` |
| `browsermob.serviceType` | The Service type | `NodePort` |
| `browsermob.serviceSessionAffinity` | The session affinity for the hub service| `ClientIP` |
| `browsermob.timeZone` | The time zone for the container | `nil` |
| `browsermob.nodeselector` | Node label to use for scheduling of the hub if set this takes precedence over the global value | `nil` |
...
```console
$ helm install --name my-release -f values.yaml selenium-browsermob
```
